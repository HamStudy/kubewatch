	...
	tests := []struct {
	...
	}{
		{
			...
			validateRows: func(t *testing.T, rows []table.Row) {
				assert.Equal(t, table.Row{"pod1", "Running", "default"}, rows[0])
				assert.Equal(t, table.Row{"pod2", "Pending", "default"}, rows[1])
				assert.Equal(t, table.Row{"pod3", "Failed", "default"}, rows[2])
			},
		},
		{
		...
		},
		{
			...
			validateRows: func(t *testing.T, rows []table.Row) {
				assert.Equal(t, table.Row{"pod1", "Pod"}, rows[0])
				assert.Equal(t, table.Row{"deploy1", "Deployment"}, rows[1])
			},
		},
	}
